...
class Environment(layout: String) {
  ...
  private def composeCurrentState(): WumpusPercept = {
    ...
    val replics = Array("I feel/see ", "There is/are ", "It's something here, exactly ")
    for (r <- adjacentRooms) {
      if (wumpusPositions == r)
      {stench = true
        Console.println(Random.shuffle(replics.toList).head + "stench")}
      if (pitPosition == r)
        {breeze = true
          Console.println(Random.shuffle(replics.toList).head + "breeze")}
    }
    if (pos == goldPosition)
      {glitter = true
        Console.println(Random.shuffle(replics.toList).head + "glitter")}
    if (wumpusJustKilled)
      {scream = true
        Console.println(Random.shuffle(replics.toList).head + "scream")}
